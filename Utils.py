...
class layers:
    ...
    def weights(self,prevLayerSize):
        ...
        # He Initialization formula: std = sqrt(2 / n_in)
        std_dev = np.sqrt(2 / prevLayerSize)
        
        # Initialize weights from a Gaussian distribution
        # Shape is (currentLayerSize, prevLayerSize)
        self.W = np.random.normal(loc=0.0, scale=std_dev, 
                                size=(self.layerSize, prevLayerSize))
